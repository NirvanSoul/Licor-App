...
export default function InventoryFab() {
    ...
    const handleConfirm = async () => {
        ...
        // Logic check: Are we ONLY subtracting items?
        // If so, we don't need cost inputs. Just commit.
        const allNegative = Object.values(pendingInventory).every(qty => qty <= 0);
        ...
        // If Adding Stock (at least one positive) -> Show Cost Modal for POSITIVE items only
        const initialCosts = {};
        Object.entries(pendingInventory).forEach(([key, qty]) => {
            if (qty > 0) { // Only ask cost for items we are ADDING
                const parts = key.split('_');
                const emission = parts.pop();
                const subtype = parts.pop();
