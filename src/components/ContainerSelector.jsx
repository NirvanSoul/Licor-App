...
export default function ContainerSelector({ value, onChange }) {
    // Parse current value
    const isCan = value.includes('Lata');
    const isTercio = value.includes('Tercio');
    const isLarge = value.includes('Grande');
    ...
    const SlidingSegmentedControl = ({ options, onSelect }) => {
        ...
        return (
            <div style={{
                position: 'relative',
                display: 'flex',
                background: '#E5E5EA', // User requested #E5E5EA
                borderRadius: '9999px',
                ...
                {options.map((opt, index) => (
                    <button
                        ...
                        style={{
                            ...
                            color: opt.isActive ? '#ffffff' : '#6b7280',
                            fontWeight: 600,
    ...
    return (
        <div className="container-selector" style={{
            ...
            gap: '8px',
            minWidth: '200px' // Ensure enough horizontal breathing room for text
        }}>

            {/* Level 1: Main Type (Botella vs Lata) */}
            <SlidingSegmentedControl
                onSelect={(opt) => {
                    if (opt.value === 'Botella') onChange('Botella');
                    else onChange('Lata Pequeña');
                }}
                options={[
            ...
            {/* Minimal gap styling handled by parent flex gap, but we can animate height/opacity if we wanted extra polish.
                For now, we stick to the crisp conditional rendering. */}
            <div style={{
                ...
                {isCan ? (
                    ...
                    <SlidingSegmentedControl
                        onSelect={(opt) => onChange(opt.value === 'Tercio' ? 'Botella Tercio' : 'Botella')}
                        options={[
                            { label: 'Estándar', value: 'Standard', isActive: !isTercio },
                            { label: 'Tercio', value: 'Tercio', isActive: isTercio }
                        ]}
                    />
                )}
            </div>
