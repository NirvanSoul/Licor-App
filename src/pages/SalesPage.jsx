...
export default function SalesPage() {
    ...
    const {
        ...
        getInventory, getEmissionsForSubtype, currencySymbol, beerCategories
    } = useProduct();
    ...
    // const [showModal, setShowModal] = useState(false); // MOVED UP
    ...
    const handleFinalConfirm = () => {
        ...
        if (ticketStep === 2) {
            // STRICT PAYMENT VALIDATION ONLY FOR TAKEAWAY
            if (hasTakeAway) {
                        ...
                        <div className="options-grid">
                            {beerTypes
                                .filter(beer => {
                                    ...
                                    // 1. Check stored category
                                    if (category) return category.toLowerCase().includes(activeSubtype.includes('lata') ? 'lata' : 'botella');

                                    // 2. Fallback for legacy
                                    const price = getPrice(beer, 'Unidad', orderState.subtype);
                                    if (price > 0) return true;

                                    // 3. Last fallback (if nothing found, show it anyway to avoid empty list for old data?)
                                    // Actually better to be strict if we have categories
                                    return false;
                                })
                                .map(opt => (
                                    <button
